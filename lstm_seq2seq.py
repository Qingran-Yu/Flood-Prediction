...
RESULT_DIR = os.path.join(BASE_DIR, "results_lstm")
...
# ===================== Physical Constraint Loss (Simplified for LSTM Baseline) =====================
def physical_constraint_loss(preds_scaled: torch.Tensor, probe_coords: np.ndarray, scaler: StandardScaler) -> torch.Tensor:
    """
    Physical constraint loss: only velocity bounds are applied to keep the baseline fair.
    The full physical loss (gradient, direction, smoothness) is used in the main model.
    """
...
# ===================== Visualization Functions (Aligned with Main Code) =====================
...
def load_and_preprocess_data() -> Tuple[np.ndarray, np.ndarray, None, int]:
    """
    ...
    return feat_tensor, probe_coords, scaler, num_probes
...
class EncoderLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor], int, int]:
        B, T_in, N, C = x.shape
        self.N = N
        self.C = C
        x_reshaped = x.reshape(B, T_in, -1)
        enc_out, (h_n, c_n) = self.lstm(x_reshaped)
        enc_out = self.norm(enc_out)
        return enc_out, (h_n, c_n), N, C

class DecoderLSTM(nn.Module):
    """LSTM decoder with attention."""
    def __init__(self, feat_dim: int, hid_dim: int, out_dim: int, attn_dim: int, n_layers: int = 2, dropout: float = 0.1):
        super().__init__()
        self.attn = BahdanauAttention(hid_dim, attn_dim)
        self.lstm = nn.LSTM(
            input_size=feat_dim + hid_dim,
            hidden_size=hid_dim,
            num_layers=n_layers,
            batch_first=True,
            dropout=dropout if n_layers > 1 else 0
    ...
    def forward(self, x_t: torch.Tensor, enc_out: torch.Tensor,
        ...
        ctx = ctx.unsqueeze(1)
        lstm_input = torch.cat([x_seq, ctx], dim=-1)
...
class PureLSTM(nn.Module):
    """Pure LSTM Seq2Seq model with attention (no spatial graph)."""
    def __init__(self, node_feat_dim: int, hidden_dim: int, out_dim: int, out_seq: int, attn_dim: int, num_probes: int):
        super().__init__()
        self.out_seq = out_seq
        self.flat_feat_dim = num_probes * node_feat_dim
        self.encoder = EncoderLSTM(self.flat_feat_dim, hidden_dim)
        self.decoder = DecoderLSTM(self.flat_feat_dim, hidden_dim, out_dim * num_probes, attn_dim)
        self.init_proj = nn.Linear(hidden_dim, self.flat_feat_dim)

    def forward(self, x: torch.Tensor, target: Optional[torch.Tensor] = None) -> torch.Tensor:
        B = x.shape[0]
        enc_out, (h, c), N, C = self.encoder(x)
        x_t = self.init_proj(h[-1])
        outputs = []
        for t in range(self.out_seq):
            x_t, (h, c) = self.decoder(x_t, enc_out, h, c)
            outputs.append(x_t)
            if self.training and target is not None and torch.rand(1).item() < Config.TEACHER_FORCING_RATIO:
                x_t = target[:, t].reshape(B, -1)
        outputs = torch.stack(outputs, dim=1)
        final_out = outputs.reshape(B, self.out_seq, N, C)
        return final_out

# ===================== Main Training Pipeline =====================
def train_and_predict() -> None:
    """Main training pipeline for pure LSTM baseline."""
    set_random_seed(42)

    feat_tensor, probe_coords, _, num_probes = load_and_preprocess_data()
    ...
    print("\n" + "="*20 + " Start Training Pure LSTM " + "="*20)
    for epoch in range(Config.EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(RESULT_DIR, "best_lstm_model.pth"))
    ...
    model.load_state_dict(torch.load(os.path.join(RESULT_DIR, "best_lstm_model.pth")))
    model.eval()
    test_preds = []
    test_targets = []
    with torch.no_grad():
